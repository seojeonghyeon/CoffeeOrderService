...
class ProductApiControllerTest {
    ...
    private static final String ROOT = "/api/order/products";
    private static final String PRODUCT_SEARCH = "/search";
    private static final String PRODUCT_POPULAR_TOP_THREE_BY_LAST_SEVEN_DAYS = "/popular/products";
    ...
    void items() throws Exception{
        mockMvc
                ...
                .andExpect(jsonPath("responseProductListItems").exists());
    }

    @DisplayName("Drink 상세 정보")
    @Test
    void tea() throws Exception{
        mockMvc
                ...
                .andExpect(jsonPath("productName").value("Americano(Hot)"))
                .andExpect(jsonPath("price").value(2000))
                .andExpect(jsonPath("quantity").value(10000))
                .andExpect(jsonPath("productImage").value("https://cdn.paris.spl.li/wp-content/uploads/200406_HOT%E1%84%8B%E1%85%A1%E1%84%86%E1%85%A6%E1%84%85%E1%85%B5%E1%84%8F%E1%85%A1%E1%84%82%E1%85%A9-1280x1280.jpg"));
    }
    ...
    void search() throws Exception{
        mockMvc
                .perform(
                        get(ROOT + PRODUCT_SEARCH +"?minPrice=2300&maxPrice=2900&page=1&size=1")
                )
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("responseProductSearchList").exists())
                .andExpect(jsonPath("$.responseProductSearchList[0].productName").value("Caffe Latte(Ice)"));
    }
    ...
    void popularTeas() throws Exception{
        ...
        mockMvc
                .perform(
                        get(ROOT + PRODUCT_POPULAR_TOP_THREE_BY_LAST_SEVEN_DAYS)
                )
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("responsePopularProductList").exists())
                .andExpect(jsonPath("$.responsePopularProductList[0].id").value(5));
    }
}